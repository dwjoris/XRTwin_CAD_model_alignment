...
Run PointNetLK, with given parameters, for given .h5 file
...
# Testing imports
from _test.tester import test_one_epoch, test_one_epoch_PNLK_partial
...
def test(args, model, test_loader, DIR):
    reg_time = test_one_epoch(args.device, model, test_loader, DIR, algo="PointNetLK")
    #test_one_epoch_PNLK_partial(args.device, model, test_loader, nmb_it=50)
    return reg_time
...
def main(h5_file_loc,object_name,zero_mean=True,voxel_size=0):
    ...
    # dataset = RegistrationData('PointNetLK', ModelNet40Data(train=False),additional_params={'use_masknet': False},noise=True) #change for own data
    ...
    model = PointNetLK(feature_model=ptnet,pooling="max",
                       p0_zero_mean = zero_mean, p1_zero_mean = True) #avg much better than #max for noisy data
