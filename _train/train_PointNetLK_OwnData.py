        ...
        for i, data in enumerate(tqdm(test_loader)):
            ...
            if(loss_val.item() > Err_Thres and epoch > Epoch_Lim): #Error cases to investigate later are saved
                write_error_h5(template,source,igt,output['est_T'],output['transformed_source'], 
                               epoch, 'test')
                print(FrobeniusNormLoss()(output['est_T'], igt))
                print(RMSEFeaturesLoss()(output['r']))
...
def train_one_epoch(device, model, train_loader, optimizer, epoch): #similar to test_one_epoch
    ...
    for i, data in enumerate(tqdm(train_loader)):
        ...
        loss_val = FN_loss + RMSEFeat_loss
...
def main():
    ...
    "-------------------------"
    #args.resume = "C:/Users/menth/Documents/Python Scripts/Thesis/toolboxes/learning3d/checkpoints/exp_pnlk/models/model_snap.t7"
    
    "------SETUP LOADING------"
    # trainset = UserData('registration',file_train)
    # testset = UserData('registration',file_test)
    
    trainset = RegistrationData('PointNetLK', ModelNet40Data(train=True),additional_params={'use_masknet': False})
    testset = RegistrationData('PointNetLK', ModelNet40Data(train=False),additional_params={'use_masknet': False})
    ...
    if not torch.cuda.is_available(): #GPU available?
        args.device = 'cpu'
    ...
    if args.eval:
        test(args, model, test_loader, textio) #Evaluating => test
    else:
